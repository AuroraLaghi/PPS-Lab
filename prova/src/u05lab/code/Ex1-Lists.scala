...
trait ListImplementation[A] extends List[A] {
  ...
  override def zipRight: List[(A,Int)] = {
    def _zipRight(l:List[A], acc: Int) : List[(A,Int)] = l match {
      case Cons(h,t) => Cons((h,acc), _zipRight(t, acc + 1))
      case Nil() => Nil()
    }
    _zipRight(this,0);
  }
  ...
  override def span(pred: A => Boolean): (List[A],List[A]) = {
    def _head(l: List[A], pred: A => Boolean): List[A] = l match {
      case Cons(h, t) if pred(h) => Cons(h, _head(t, pred))
      case _ => Nil()
    }

    def _tail(l: List[A], pred: A => Boolean): List[A] = l match {
      case Cons(h, t) if pred(h) => _tail(t, pred)
  ...
  override def reduce (op: (A,A)=>A): A = this match {
    case Cons(h,t) if t != Nil() => op(h,t.reduce(op))
    case Cons(h,_) => h
    case _ => throw new UnsupportedOperationException
  }

  override def takeRight(n: Int): List[A] = {
    this.reverse.zipRight.filter(_._2 < n).map(_._1).reverse()
  }
}
